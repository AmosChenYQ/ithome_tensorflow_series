...
def main(_):
  ...
  optimizer_options = tf.OptimizerOptions(cpu_global_jit=True, global_jit_level=tf.compat.v1.OptimizerOptions.ON_2)
  ...
  with tf.name_scope("train"):
    train_step = tf.train.AdamOptimizer(
        FLAGS.learning_rate, name="Adam").minimize(cross_entropy)
  ...
  frozen_graph = tf.graph_util.convert_variables_to_constants(
    sess, tf.get_default_graph().as_graph_def(), ["accuracy/accuracy/accuracy", "train/Adam"])
  tf.summary.FileWriter("events/", graph=frozen_graph)
